...
from utils import confidence_ellipse, plotData, plotDist, to_numpy, res_path, plotCaviProgress, genResDir, genGif
...
def getMogData(K: int = 5, sigma: float = 1, nDims: int = 2, nSamples: int = 100, debug: bool = True):
    ...
    if debug:
        ...
        # ax[1].set_ylim((0, 0.5))
        ax[1].grid()
        fig.savefig(Path(res_path, "dataDist.png"))

    return x_df


def CAVI(data: pd.DataFrame, sigma: float, debug=True, path=res_path):
    ...
    if debug:
        plotCaviProgress(data, m, s2, path=path)
    ...
    elbo = []
    iter = 0
    while not is_conv and iter < 100:
        ...
        if debug:
            plotCaviProgress(data, m, s2, iter, path=path)
    ...
    if debug:
        ...
        ax0.grid()
        fig0.savefig(Path(path, "elbo.png"))
        plt.close(fig0)
